...
int main() {
    ...
    using Tree = KDTree<double, size_t, 3>;
    using Pt   = Tree::Point;
    ...
    for (size_t i = 0; i < split; ++i)
        bulk.push_back({ Pt(pts[i][0], pts[i][1], pts[i][2]), i });
    ...
    for (size_t i = split; i < pts.size(); ++i)
        tree.insert(Pt(pts[i][0], pts[i][1], pts[i][2]), i);
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        size_t before = tree.treeSize;
        tree.remove(p);
        if (tree.treeSize < before) ++del_count;
    }
    ...
    // kNN query
    vector<size_t> knn;
    tree.knnQuery(Pt(539.271901, 2915.970570, -21.334010), 10, back_inserter(knn));
    cout << "10-NN query returned " << knn.size() << " results\n";

    // Statistics
    auto s = tree.getStatistics();
    cout << "\n=== KD-tree (3D) statistics ===\n";
