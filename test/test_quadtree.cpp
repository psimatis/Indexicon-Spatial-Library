...
int main() {
    ...
    using Tree = QuadTreeNode<double, size_t>;
    ...
    for (size_t i = 0; i < split; ++i)
        bulk.push_back(Pt(i, pts[i][0], pts[i][1]));
    ...
    for (size_t i = split; i < pts.size(); ++i)
        tree.insert(Pt(i, pts[i][0], pts[i][1]));
    cout << "Inserted " << (pts.size() - split) << " more points\n";

    // Delete
    size_t del_count = 0;
    for (size_t i = 0; i < 100; ++i)
        if (tree.remove(Pt(i, pts[i][0], pts[i][1]))) ++del_count;
