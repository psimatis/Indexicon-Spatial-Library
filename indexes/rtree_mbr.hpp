#ifndef RTREE_MBR_HPP
#define RTREE_MBR_HPP
...
template <typename Value, typename BoxType, size_t MaxCap>
struct LeafNode : Node<Value, BoxType> {
    FixedVector<pair<BoxType, Value>, MaxCap> elements;

    LeafNode() : Node<Value, BoxType>(true) {}
};

template <typename Value, typename BoxType, size_t MaxLeafCap, size_t MaxInternalCap>
    ...
    ~InternalNode() {
        for (auto& child : children) {
            if (child.second->isLeaf)
                delete static_cast<LeafNode<Value, BoxType, MaxLeafCap>*>(child.second);
            else
                delete static_cast<InternalNode<Value, BoxType, MaxLeafCap, MaxInternalCap>*>(child.second);
        }
    }
};
...
public:
    ...
    using LeafType = LeafNode<Value, BoxType, MaxElements + 1>;
    using InternalType = InternalNode<Value, BoxType, MaxElements + 1, MaxFanout + 1>;
    ...
    RTree(Iter first, Iter last) : root(nullptr), size(0), height(0) {
        ...
        for (Iter it = first; it != last; ++it) {
            ...
            entries.push_back({it->first.center(), it});
        }
    ...
    ~RTree() {
        if (root->isLeaf)
            delete static_cast<LeafType*>(root);
        else
            delete static_cast<InternalType*>(root);
    }

    void insert(const BoxType& bounds, const Value& value) {
        vector<pair<BoxType, Value>> leafReinserts;
        vector<tuple<BoxType, NodeType*, size_t>> internalReinserts;

        NodeType* splitNode = insertImpl(root, bounds, value, height, leafReinserts, internalReinserts);
        ++size;
    ...
    NodeType* insertImpl(NodeType* node, const BoxType& bounds, const Value& value, size_t level, vector<pair<BoxType, Value>>& leafReinserts, vector<tuple<BoxType, NodeType*, size_t>>& internalReinserts) {
        if (node->isLeaf) {
            auto* leaf = static_cast<LeafType*>(node);
            leaf->elements.push_back({bounds, value});
            ...
            size_t bestIdx = chooseSubtree(internal, bounds, level - 1);
            NodeType* childSplit = insertImpl(internal->children[bestIdx].second, bounds, value, level - 1, leafReinserts, internalReinserts);
    ...
    void reinsertLeaf(LeafType* leaf, vector<pair<BoxType, Value>>& reinsertList) {
        ...
        for (size_t i = 0; i < elements.size(); ++i)
            sorted[i] = {squaredDistance(elements[i].first.center(), center), i};
        ...
        FixedVector<pair<BoxType, Value>, MaxElements + 1> remaining;
    ...
    NodeType* insertNoLeafReinsert(NodeType* node, const BoxType& bounds, const Value& value, size_t level, vector<tuple<BoxType, NodeType*, size_t>>& internalReinserts) {
        if (node->isLeaf) {
            auto* leaf = static_cast<LeafType*>(node);
            leaf->elements.push_back({bounds, value});
            leaf->bounds.expand(bounds);
            ...
            auto* internal = static_cast<InternalType*>(node);
            size_t bestIdx = chooseSubtree(internal, bounds, level - 1);
            NodeType* childSplit = insertNoLeafReinsert(internal->children[bestIdx].second, bounds, value, level - 1, internalReinserts);
    ...
    static void computePrefixSuffix(Container& items, BoxContainer& prefix, BoxContainer& suffix, size_t axis, bool useMax) {
        if (useMax)
    ...
    size_t chooseSubtreeOverlap(InternalType* node, const BoxType& bounds) {
        ...
        if (bestEnlargement <= numeric_limits<Type>::epsilon()) // If best enlargement is tiny return early
            return bestIdx;
    ...
    static void partition(It first, It nth, It last, size_t axis, bool use_max) {
        if (use_max)
            nth_element(first, nth, last, [axis](const auto& a, const auto& b) { return a.first.maxCorner[axis] < b.first.maxCorner[axis]; });
        else
            nth_element(first, nth, last, [axis](const auto& a, const auto& b) { return a.first.minCorner[axis] < b.first.minCorner[axis]; });
    }

    NodeType* splitLeaf(LeafType* leaf) {
        auto& elements = leaf->elements;
        
        FixedVector<pair<BoxType, Value>, MaxElements + 1> elements_copy;
        elements_copy.assign(elements.begin(), elements.end());
        const size_t n = elements_copy.size();

        bool isPointData = true;
        for (size_t d = 0; d < Dims; ++d) {
            if (elements_copy[0].first.minCorner[d] != elements_copy[0].first.maxCorner[d]) {
                isPointData = false;
                break;
            }
        }
        ...
        for (size_t axis = 0; axis < Dims; ++axis) {
            Type marginSum = 0;
            computePrefixSuffix(elements_copy, prefix, suffix, axis, false);
            for (size_t split = minElements; split <= n - minElements; ++split)
                marginSum += prefix[split].margin() + suffix[split].margin();
            if (!isPointData) {
                computePrefixSuffix(elements_copy, prefix, suffix, axis, true);
                for (size_t split = minElements; split <= n - minElements; ++split)
                    marginSum += prefix[split].margin() + suffix[split].margin();
            }
        ...
        computePrefixSuffix(elements_copy, prefix, suffix, bestAxis, false);
        ...
        if (!isPointData) {
            computePrefixSuffix(elements_copy, prefix, suffix, bestAxis, true);
        ...
        partition(elements.begin(), elements.begin() + bestSplit, elements.end(), bestAxis, bestUseMax);
    ...
    NodeType* splitInternal(InternalType* internal) {
        ...
        for (size_t axis = 0; axis < Dims; ++axis) {
            Type marginSum = 0;
            computePrefixSuffix(children_copy, prefix, suffix, axis, false);
            for (size_t split = minFanout; split <= n - minFanout; ++split)
                marginSum += prefix[split].margin() + suffix[split].margin();
            computePrefixSuffix(children_copy, prefix, suffix, axis, true);
            for (size_t split = minFanout; split <= n - minFanout; ++split)
                marginSum += prefix[split].margin() + suffix[split].margin();
            if (marginSum < bestMarginSum) {
                bestMarginSum = marginSum;
        ...
        computePrefixSuffix(children_copy, prefix, suffix, bestAxis, false);
        ...
        computePrefixSuffix(children_copy, prefix, suffix, bestAxis, true);
        ...
        partition(children.begin(), children.begin() + bestSplit, children.end(), bestAxis, bestUseMax);
    ...
    bool remove(const BoxType& bounds, const Value& value) {
        vector<pair<BoxType, Value>> orphans;
        bool found = removeImpl(root, bounds, value, orphans);
    ...
    bool removeImpl(NodeType* node, const BoxType& bounds, const Value& value, vector<pair<BoxType, Value>>& orphans) {
        if (node->isLeaf) {
            auto* leaf = static_cast<LeafType*>(node);
            for (auto it = leaf->elements.begin(); it != leaf->elements.end(); ++it) {
                if (it->second == value && intersects(it->first, bounds)) {
            ...
            for (size_t i = 0; i < internal->children.size(); ++i) {
                if (intersects(internal->children[i].first, bounds)) {
                    if (removeImpl(internal->children[i].second, bounds, value, orphans)) {
                        ...
                        if (child_size < (child->isLeaf ? minElements : minFanout) && internal->children.size() > 1) {
                            ...
                            if (child->isLeaf)
                                delete static_cast<LeafType*>(child);
                            else
                                delete static_cast<InternalType*>(child);
    ...
    void collectOrphans(NodeType* node, vector<pair<BoxType, Value>>& orphans) {
        if (node->isLeaf) {
            auto* leaf = static_cast<LeafType*>(node);
    ...
    void queryImpl(const NodeType* node, const BoxType& query_box, OutIter& out) const {
        if (node->isLeaf) {
            ...
            if (contains(query_box, node->bounds)) {
                ...
                for (const auto& elem : elems) {
                    if (intersects(elem.first, query_box)) 
                        *out++ = elem.second;
                }
            }
        } else {
    ...
    void knnQuery(const PointType& query, size_t k, OutIter out) const {
        ...
        size_t rl = height - 1;  // reverse_level: 0=leaf, root=height_-1
        while (true) {
            if (rl > 0) {
                ...
                for (const auto& elem : leaf->elements) {
                    Type d = mindistPointToBox(query, elem.first);
                    if (neighbors.size() < k) {
